  ...
  return (
    <div className="space-y-6 select-none font-sans relative">
      ...
      <div className="relative z-30 flex flex-col md:flex-row md:items-center justify-between gap-4 p-5 sm:p-6 rounded-[28px] bg-white/90 backdrop-blur-xl border border-stone-200/80 shadow-[0_10px_35px_-10px_rgba(0,0,0,0.04)]">
        ...
        {/* Action Controls: Mobile Stack (Vertical Full Width) & Desktop Inline Row */}
        <div className="w-full md:w-auto flex flex-col sm:flex-row items-stretch sm:items-center gap-2.5 relative">
          
          {/* Direct Restore Button (Full Width on Mobile) */}
          <motion.button
            whileTap={{ scale: 0.98 }}
            whileHover={{ scale: 1.01 }}
            type="button"
            onClick={() => setIsRestoreModalOpen(true)}
            className="w-full sm:w-auto px-5 py-2.5 rounded-full bg-white hover:bg-stone-50 text-zinc-800 border border-stone-300/90 hover:border-orange-500/40 text-xs font-mono font-bold flex items-center justify-center gap-2 shadow-2xs transition-all cursor-pointer"
          >
            <Upload className="w-3.5 h-3.5 text-orange-600 shrink-0" />
            <span>Restore Backup</span>
          </motion.button>

          {/* Export Suite Dropdown (Full Width on Mobile) */}
          <div className="relative w-full sm:w-auto">
            ...
            >
              ...
              <ChevronDown className={`w-3 h-3 text-stone-400 shrink-0 transition-transform ${isExportMenuOpen ? "rotate-180" : ""}`} />
            </motion.button>
            ...
            <AnimatePresence>
              {isExportMenuOpen && (
                <motion.div
                  ...
                  className="absolute right-0 top-full mt-2 w-full sm:w-64 max-w-[calc(100vw-40px)] p-2 rounded-2xl bg-white/95 backdrop-blur-2xl border border-stone-200/90 shadow-[0_20px_50px_rgba(0,0,0,0.18)] z-[100] space-y-1 font-mono text-xs"
                >
                  <button
                    type="button"
      ...
      <BotanicalCradle variant="emerald" floatDelay={0.2}>
        <div className="relative z-10 rounded-[32px] bg-white border border-stone-200/80 shadow-[0_15px_40px_-15px_rgba(0,0,0,0.05)] overflow-hidden">
          {filteredTransactions.length === 0 ? (
            ...
            <>
              {/* 1. Mobile Adaptive Card List (Screens < 640px) */}
              ...
              {/* 2. Desktop Full Matrix Table (Screens >= 640px) */}
              <div className="hidden sm:block overflow-x-auto no-scrollbar [transform:translateZ(0)]">
                <table className="w-full text-left font-mono text-xs border-collapse">
                  <thead>
                    <tr className="border-b border-stone-200/80 bg-stone-50/90 text-stone-500 text-[10px] uppercase tracking-wider font-bold">
                      <th className="py-3.5 px-5">Entry & Merchant</th>
                      <th className="py-3.5 px-4">Partition Vault</th>
                      <th className="py-3.5 px-4">Date</th>
                      <th className="py-3.5 px-4">Audit Tag</th>
                      <th className="py-3.5 px-4 text-right">Amount ({currentCurrency})</th>
                  ...
                  <tbody className="divide-y divide-stone-100">
                    <AnimatePresence mode="popLayout">
                      {filteredTransactions.map((tx) => {
                        const isInflow = tx.type === "income";
                        const displayTitle = tx.title || tx.merchant || tx.descriptor || "Transaction Entry";
                        const displaySub = tx.category ? `${tx.category} • ${tx.merchant || "Internal"}` : "Internal Vault";
                          ...
                          >
                            <td className="py-3.5 px-5">
                              <div className="flex items-center gap-3">
                                ...
                                <div className="truncate">
                                  <p className="font-bold text-zinc-950 font-sans text-sm tracking-tight truncate leading-tight">
                                    {displayTitle}
                                  </p>
                                  <span className="text-[10px] text-stone-500 capitalize font-mono block mt-0.5">
                                    {displaySub}
                                  </span>
                                </div>
                              </div>
                            ...
                            <td className="py-3.5 px-4 text-zinc-800 font-semibold">
                              <span className="px-2.5 py-0.5 rounded-full bg-stone-100 border border-stone-200/80 text-[10px] capitalize font-medium text-stone-700">
                                {tx.vaultId || "primary"}
                              </span>
                            </td>

                            <td className="py-3.5 px-4 text-zinc-600 text-[11px] font-medium">
                              {formatDate(tx.date)}
                            </td>
                            ...
                            <td
                              className={`py-3.5 px-4 text-right font-black text-sm tracking-tight ${
                                isInflow ? "text-emerald-600" : "text-zinc-950"
                              }`}
                            >
                              {isInflow ? "+" : "-"}
      ...
      <RestoreModal
        ...
        onRestoreSuccess={(restoredData) => {
          ...
          showToast("Ledger successfully restored from archive");
        }}
      />
    </div>
  );
